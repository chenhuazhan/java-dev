...
//主类
public class Sample25_6 extends JFrame {
	// 创建自定义表格模型对象
	...
	private class MyTableModel extends AbstractTableModel {
		// 创建表示各个列类型的类型数组
		Class[] typeArray = { Object.class, Double.class, Boolean.class, Icon.class, Color.class };
		// 创建列标题字符串数组
		String[] head = { "Object", "Number", "Boolean", "Icon", "Color" };
		// 创建初始表格数据
		Object[][] data = { { "这里是字符串1", new Integer(20), new Boolean(true), new ImageIcon("D:/p1.gif "), Color.black },
				{ "这里是字符串2", new Double(12.34), new Boolean(false), new ImageIcon("D:/p2.gif "), Color.darkGray },
				{ "这里是字符串3", new Integer(125), new Boolean(true), new ImageIcon("D:/p3.gif"), Color.white } };
	...
	// 自定义的绘制器
	private class MyCellRendererForColorClass extends JLabel implements TableCellRenderer {
		// 定义构造器
	...
	// 声明Color类型的自定义编辑器
	class MyEditorForColorClass extends AbstractCellEditor implements TableCellEditor, ActionListener {
		// 定义Color变量
		Color c;
		// 定义对话框变量
		JDialog jd;
	...
	// 声明Icon类型的自定义编辑器
	class MyEditorForIconClass extends AbstractCellEditor implements TableCellEditor, ActionListener {
		// 定义Icon变量
		Icon icon;
		// 创建一个按扭
		JButton jb = new JButton();
